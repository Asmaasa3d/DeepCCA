...
def cca_loss(outdim_size, use_all_singular_values):
    ...
    def inner_cca_objective(y_true, y_pred):
        ...
        SigmaHat11RootInv = tf.matmul(tf.matmul(V1, tf.linalg.tensor_diag(D1 ** -0.5)), V1, transpose_b=True)  # [dim, dim]
        SigmaHat22RootInv = tf.matmul(tf.matmul(V2, tf.linalg.tensor_diag(D2 ** -0.5)), V2, transpose_b=True)
